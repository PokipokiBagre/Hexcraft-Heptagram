...
export async function cargarTodo() {
    try {
        // 1. Cargar Estadísticas HEX (Prioridad Máxima)
        const resS = await fetch(URL_S + "&cache=" + Date.now());
        ...
        // 2. Cargar Objetos (Para dueños faltantes como Yuko o Volvo)
        const resO = await fetch(URL_O + "&cache=" + Date.now());
        ...
        filasO.slice(1).forEach(f => {
            ...
            // Marcar como principales si tienen inventario
            dueños.forEach(d => { if(d && !estadoUI.principales.includes(d)) estadoUI.principales.push(d); });
        });

        guardar();
        return true;
    } catch (e) { console.error("Error crítico de fusión:", e); return false; }
}
